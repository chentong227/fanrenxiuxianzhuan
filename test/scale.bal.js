...
// 测试用同档法术：tier 随玩家境界动态设置（验证同阶招式÷敌血恒定）
const SPELLS = require("../js/combat.js").SPELLS;
SPELLS._test_atk = { name: "测试攻击", mp: 8, range: [1, 4], type: "atk", dmg: 24, source: "art", tier: 0 };
SPELLS._test_heal = { name: "测试治疗", mp: 10, range: [0, 0], type: "heal", heal: 11, source: "art", tier: 0 };
SPELLS._test_shield = { name: "测试护体", mp: 10, range: [0, 0], type: "def", shield: 16, source: "art", tier: 0 };

function tierPlayer(tier) {
  // 同档法术：tier 随玩家境界走——验证"同阶好招占敌血恒定"
  SPELLS._test_atk.tier = tier;
  SPELLS._test_heal.tier = tier;
  SPELLS._test_shield.tier = tier;
  return new Fighter({
    ...
    spells: ["_test_heal", "_test_shield", "_test_atk"],
  });
}
...
/* ---- 断言①：好招÷敌血·轴内恒定 ---- */
console.log("\n[\u2460 \u597d\u62db\u00f7\u654c\u8840\u00b7\u8f74\u5185\u6052\u5b9a]");
// 分级制：spellPower 按 itemTier 缩放——同阶招式÷同阶敌血恒定
const r0 = B.spellPower(42, "art", 1, 0, 1, 0) / (100 * B.realmBand(0));
let maxDev = 0;
for (const t of [0, 1, 2, 3, 4]) {
  const ratio = B.spellPower(42, "art", 1, t, 1, t) / (100 * B.realmBand(t));
...
const newQi = B.spellPower(baseMove, "art", 1, 0, 1, 0) / (100 * B.realmBand(0));
const newYj = B.spellPower(baseMove, "art", 1, 3, 1, 3) / (100 * B.realmBand(3));
