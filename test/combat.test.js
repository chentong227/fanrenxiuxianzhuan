...
console.log("\n=== 1. 灵力池：整场不刷新、凝息回元亮破绽 ===");
{
  ...
  c.cast("huodan", 0);          // 耗10
  assert(c.player.mp === mp0 - 10, `火弹术耗灵力10（${mp0}→${c.player.mp}）`);
  c.endRound(); c.startRound();
  assert(c.player.mp === mp0 - 10, "回合开始灵力不自动恢复（池制）");
  const r = c.cast("ningshen", 0);
  assert(r.ok && c.player.mp === Math.min(c.player.mpMax, mp0 - 10 + 14), "凝息回元+14（不超池上限）");
