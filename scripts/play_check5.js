/**
 * UI 重叠检查 v5——用 evaluate 跳过剧情，检查地图 UI
 */
...
(async () => {
  ...
  async function checkOverlap() {
    return await page.evaluate(() => {
      ...
      const ids = ['worldmap-canvas', 'worldmap-svg', 'worldmap-pins', 'worldmap-labels',
                   ...
                   'modal-overlay', 'sheet-overlay', 'story-overlay', 'narrative-overlay'];
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const hasChoices = await evalJS(() => {
      ...
      if (!box) return 0;
      const btns = box.querySelectorAll('button.choice, button');
      return btns.length;
    });
    if (hasChoices > 0) {
      console.log(`  有 ${hasChoices} 个选项，选第一个`);
      await evalJS(() => {
        const box = document.getElementById('story-choices');
        if (box) {
          const btn = box.querySelector('button.choice, button');
          if (btn) btn.click();
        }
      });
      await wait(1000);
