...
const QUESTS = {
  ...
  tasks: {
    ...
    // 设计：以四灵根之资，单凭《长春功》苦修，约半年余可满练气一层、再图突破即达练气二层。
    //       故目标定为练气二层、限一年——不靠小绿瓶也能达成，但需稳扎稳打，留出真实的紧迫感（不再宽到失去意义）。
    modafu_deadline: {
      title: "墨大夫的期许",
      desc: "墨大夫初授《长春功》，限你一年内修至练气二层验功。届时按例重赏，莫要懈怠。",
      // 暗修四层目标在身时，点明二层是其途中点（免玩家纠结先做哪个，见 M9）
      progress: (s) => `练气${s.realmIndex + 1}层 / 需练气二层` + ((s.bottle && s.bottle.unlocked) ? "（暗修四层途中顺路达成）" : ""),
      cond: (s) => s.realmIndex >= 1,                 // 练气二层（凡修一年可达，需用功）
      onSuccess(s) {
        s.silver += 30;
      ...
      onFail(s) {
        ...
        Engine.log("【任务失败·墨大夫的期许】一年期满，你修为竟还不及练气二层。墨大夫微微摇头，眼里掠过一丝难辨的情绪。", "bad");
