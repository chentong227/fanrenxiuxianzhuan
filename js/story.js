...
const STORY = [
  ...
  {
    id: "mo_crisis",
    cond: (s) => s.flags.mo_met && !s.flags.ouyang_dead && (s.flags.mo_months || 0) >= 1,
    objTitle: "墨府客居",
    objHint: "在墨府住下（嘉元城内调息/修炼度月）——树欲静而风不止，独霸山庄不会安分太久。",
    title: "墨府 · 山雨欲来",
    text: [
      { scene: "墨府 · 正厅" },
      "你在墨府住了些时日，寒毒一日重过一日——运功时丹田如坠冰窟。",
      "这日清晨，府里突然乱了。砸门声、呵斥声，仆役们抱头乱窜。",
      "厅外站着一排彪悍武人，为首一人五十上下，紫面虬髯，按刀而立，嗓门震得梁上落灰。",
      { say: "欧阳飞天", text: "墨居仁死了，惊蛟会群龙无首——这嘉元城的地盘，往后归我独霸山庄！识相的，把墨府产业让出来，再把小姐许给我家公子，两家结好！" },
      { say: "墨彩环", tone: "压着哭腔", text: "做梦！我爹尸骨未寒，你们这群豺狗就敢上门——" },
      "墨夫人脸色苍白，却挺直了背脊。她身后，墨彩环攥着拳，眼眶通红，偷偷看向你。",
      { aside: "独霸山庄，岚州三霸之一，庄主欧阳飞天——凡俗武人巅峰，粗通吐纳。墨大夫遗书所托『安顿墨家』……便是此事了。" },
    ],
    choices: [
      {
        text: "踏前一步：「墨府的事，我管了。」",
        hint: "与欧阳飞天一战（凡俗武人巅峰+庄丁群战）",
        resolve: "ouyang_fight",
      },
    ],
  },
  {
    id: "mo_resolve",
    cond: (s) => s.flags.ouyang_dead && !s.flags.han_du_cured,
    objTitle: "墨府之诺",
    objHint: "独霸山庄已除——墨家的报答，与下一程的线索，都在墨府等你。",
    title: "暖阳宝玉 · 嫁妆",
    text: [
      { scene: "墨府 · 后园" },
      "欧阳飞天伏诛，独霸山庄树倒猢狲散。嘉元城里没人再敢打墨府的主意——至少眼下没有。",
      ...
      { aside: "太南小会……下一站有了。临行之前，还有一事要定夺——曲魂幡下的张铁，带走，还是留下？" },
    ],
    onArrive(s) {
    ...
    choices: [
      {
        text: "将曲魂留在墨府，护她们周全（动漫之选）",
        hint: "失去尸傀随行；墨府从此有一道暗中的守护",
        effect(s) {
          if (s.sideUnit) {
            Engine.writeLedger("quhun_left_mo", `将曲魂（张铁遗蜕）留在墨府护卫`);
            s.sideUnit = null;
            return { text: "夜里，你将曲魂幡郑重交托。幡下的身影立在墨府檐角的阴影里，像一座沉默的碑。\n「张铁……替我护着这家人。」你轻声说。它没有回答，只是握紧了拳。\n墨彩环不知道檐上有什么，只觉得那夜之后，府里格外安心。", kind: "event" };
          }
          Engine.writeLedger("quhun_left_mo", "允诺墨府暗中照拂");
          return { text: "你在墨府四角埋下淬毒的机关暗桩，又留下一册防身的药理手记——能做的，都做了。", kind: "event" };
        },
      },
      {
        text: "带走曲魂——修仙路险，我需要它",
        hint: "保留尸傀随行；墨府失一道屏障（负因也是内容）",
        effect(s) {
          Engine.writeLedger("quhun_kept", "将曲魂带在身边，墨府只余凡人");
          return { text: "你终究没有交出曲魂幡。修仙路上步步杀机，张铁的遗蜕是你仅有的依仗之一。\n离府那日，墨彩环送到城门口，挥着手喊：「要回来看我们呀——」\n你没有回头。有些亏欠，记在心里就好。", kind: "sys" };
        },
      },
    ],
  },
