...
const STORY = [
  ...
  },
  {
    ...
    title: "京城 · 连环失踪案",
    objTitle: "查案",
    objHint: "京城散修接连失踪——查清底细，情报越足，皇宫决战越有把握。",
    text: [
      { scene: "京城 · 茶楼" },
    ...
    choices: [
      { text: "花重金买齐蒙山五友的消息，再亲自蹲茶楼、以神识窃听（情报最全）",
        effect: (s) => { s.flags.jingcheng_intel = 2; return { text: "你舍得下本钱：蒙山五友的加密茶话、各坊失踪者的时辰路径、煞气流向的蛛丝马迹——尽数摸清。贼窝的虚实，已了然于胸（情报·full）。", kind: "good" }; },
        resolve: "advance" },
  ...
  },
  {
    ...
    text: [
      ...
      { say: "墨彩环", emo: "smile", tone: "声音压得极低，笑意却抵到了眼底", text: "……黑小子。真有你的，连这儿都能撞见。" },
      ...
      // —— 【修#2·占位·建议用户亲笔】本幕最重的情感落点（动漫名台词「谢谢你，出现在我微不足道的生命里」）。
      //    我执笔留一版克制的推荐稿（对齐燕家堡重逢基调·不越界臆造情爱），并保留占位标记：
      //    若你要亲笔改写这一句/这一段，直接替换下面这条 say 的 text 即可（与 chen_remember 同模式）。
      { say: "墨彩环", emo: "smile", tone: "她忽然认真起来，声音很轻", text: "黑小子……这些年我谁都信不过，独独见着你，心里就踏实。谢谢你——出现在我这微不足道的一辈子里。【此句留白·待亲笔】" },
      { aside: "你一向木讷，此刻竟也说不出话，只重重点了点头。仙途漫漫，有些人来过、暖过，便已是难得。墨府那笔血债，从今往后，是你们两个人的事了。" },
    ],
    onArrive(s) {
      State.setFlag("modao_e3_yanhui_done");
      Engine.meetNpc("mocaihuan", "燕家堡一别，易容藏形追仇至京城五色门门下的故人——墨府之难的因果，在此续上第二章。");
      Engine.writeLedger("modao_yanhui", "馨王府夜宴重逢易容的墨彩环——她追墨家仇人至京城五色门，约你共了这桩血债（修#2·情感落点占位待亲笔）");
  ...
  },
  {
    ...
    cond: (s) => s.flags.modao_e3_yanhui_done && !s.flags.modao_e3_tieluo_done,
    cg: "jingcheng",
    bgm: "boss",
    title: "血池 · 血侍铁罗",
    objTitle: "救人",
    ...
    text: [
      ...
      { aside: "他行火，木生火——你这身木行道基这回占不到相克的便宜，是场硬仗。可血池里还有活人在等，退无可退。先逼他现出黑煞教的形迹，再说救人！" },
    ],
    onArrive(s) {
