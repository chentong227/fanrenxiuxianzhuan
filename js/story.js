...
const STORY = [
   ...
   * ============================================================ */
  {
    ...
    text: [
      ...
      "万小山陪你排在队尾，比你还紧张：「韩兄放轻松！你修为都练气中后期了，怎么也比这些没开过灵窍的娃娃强！」",
  ...
  /* ============================================================
   *  黄枫谷篇（驻地章）· 序：得丹与夺丹
   *  考据：入谷按规领一枚筑基丹 → 叶师叔（筑基期）以"后辈需筑基"为由权势威逼，
   *  用少量灵石与药草强行换走 → 韩立被迫接百药园差事；掌门不主持公道。
   *  恨点设计：先给再夺——图鉴空位+前路栏常驻，直到地火屋自炼二十颗。
   * ============================================================ */
  {
    id: "hf_register",
    cond: (s) => s.flags.huangfeng_entered && s.location === "huangfeng_gate" && !s.flags.zhuji_dan_stolen,
    objTitle: "黄枫谷报到",
    objHint: "外门弟子的名册、青衫与腰牌——还有升仙令许诺的那样东西。",
    title: "得丹 · 夺丹",
    text: [
      { scene: "黄枫谷 · 执事殿" },
      "执事殿内，管事修士验过升仙令，从玉匣中取出一枚龙眼大的乳白丹丸。丹香入鼻，你丹田里的灵力都为之一颤。",
      { say: "管事修士", text: "升仙令入谷者，按例授筑基丹一枚。练气十一层之后服用，可冲击筑基——拿好了，这是多少散修一辈子求不来的东西。" },
      "筑基丹。入手微温，你的指尖几乎在抖——筑基之路，就握在掌心里。",
      { aside: "苦修、算计、亡命——值了。都值了。" },
      "你还没把丹收进储物袋，殿外踱进来一个鹰目薄唇的青袍老者。管事修士的腰瞬间弯了下去：「叶师叔。」",
      { say: "叶师叔", tone: "皮笑肉不笑", text: "这位就是凭令入谷的小友？好福气。不过——老夫门下有个后辈，练气十一层整，正是筑基的火候。小友才几层来着？七层？" },
      "他抬手一招，一只布袋飘到你面前：里头是十几块灵石、几株普通药草。",
      { say: "叶师叔", tone: "不容置疑", text: "丹放在你手里是糟蹋。这些，换你那枚丹——公平买卖。日后你到了火候，自有机缘。" },
      "殿内落针可闻。管事修士低头扫地，仿佛什么都没听见。你看着那只布袋，又看看老者鹰隼般的眼睛——筑基期的灵压若有似无地罩着你。",
      { aside: "反抗？练气七层对筑基。告状？掌门与他师出同门。这就是修仙界——七玄门换了个更大的壳子罢了。" },
      { say: "韩立", tone: "极平静", text: "……成交。" },
      "你松开手。那枚乳白的丹丸离开掌心的一瞬，你把它的模样刻进了心里。",
      { say: "叶师叔", tone: "满意地", text: "识时务。对了，百药园缺个看园的——小友药理不错吧？就你了。好生干，谷里亏待不了你。" },
    ],
    onArrive(s) {
      State.setFlag("zhuji_dan_stolen");
      State.give("lingshi", 12);
      State.give("lingcao", 3);
      Engine.writeLedger("zhuji_dan_grudge", "入谷之日，叶师叔强换走你的筑基丹");
      Engine.addMilestone("夺丹之辱：筑基丹得而复失", "showdown");
      Engine.toast("筑基丹：得而复失（此仇此辱，记在前路）");
    },
    choices: [
      { text: "「多谢师叔抬举。」（收下布袋，领差事去）", hint: "灵石+12 药草+3——和一笔记到地火之屋才算完的账" },
    ],
  },
  {
    id: "hf_yaoyuan",
    cond: (s) => s.flags.zhuji_dan_stolen && !s.flags.yaoyuan_started,
    title: "百药园 · 马师伯",
    text: [
      { scene: "黄枫谷 · 百药园" },
      "百药园在谷东南的向阳坡上，一畦一畦的灵田顺着山势铺开，药香沁人。",
      ...
      { say: "马师伯", tone: "含糊地", text: "……西头那排青元参苗娇贵，夜里霜重，记得盖草帘。例钱在房梁上，自己取。" },
      { aside: "刀子嘴。这园子打理得一丝不苟，他对药草倒是真心。" },
    ...
    choices: [
      { text: "接过《百草谱》", hint: "百药园差事开启（行动栏）——大帆时代：种药、修炼、攒家底" },
    ],
  },
];
