...
const Engine = {
   ...
   * =========================================================== */
  hasFuluTable() { return State.count("zhifu_bi") > 0; },   // 制符台＝持制符笔方可开（考据：太南小会·菡云芝）
  // 解锁一份符箓方案（来源：购买/沟通/战胜——皆走此入口）
  ...
  // 制符：有方案 + 符纸 + 灵力即成（熟练度只影响成功率/产量，不引入新材料）
  makeFulu(planId) {
    const s = State.data;
    const plan = DATA.fuluPlans[planId];
    if (!plan) return;
