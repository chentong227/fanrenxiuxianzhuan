...
(function (root) {
  const Balance = {
    ...
    /* ---- 修为增速几何尺（polish-huangfeng A1③·2026-07-12）----
     * 修为墙随层数几何走高（culMax ×≈1.3/层），而旧闭关/丹药收益恒定＝越修越慢
     * （黄枫谷 7→11 层实测≈15 年游戏时间——"修为墙=乐观十余年"的根）。
     * 一把尺子：练气七层（realmIndex 6）起，修炼与修为类丹药收益按 ×1.62/层 几何随涨——
     * "境界越高吐纳越阔、同一枚丹炼化得越彻底（修为账随境界水涨船高）"。
     * realmIndex ≤5（练气1~6）恒 1.0 → 七玄门篇已校准曲线逐字节零扰动；
     * 练气十三层（index 12）后指数封顶（×1.62^7≈29.3）——筑基墙 6000/8200 由此落在
     * 每层约 8~18 月的可玩带，而结丹大墙仍须篇章秘仪/剧情供给节奏（几何尺不替剧情发境界）。
     * 与 realmBand（战斗标度）同哲学：几何对几何，禁线性平铺。校准回路 test/huangfeng-time.audit.js。 */
    culGainMul(realmIndex) {
      return Math.pow(1.62, clamp((realmIndex || 0) - 5, 0, 7));
    },
