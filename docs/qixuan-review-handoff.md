...
| 项 | 值 |
|---|---|
| 更新时间（现实） | 2026-06-18（checkpoint #1） |
| 角色 | 韩立（四灵根） |
| 境界 | 练气三层（realmIndex 2），修为 200 / 240（满 240 即可冲练气四层） |
| 游戏时间 | 第 3 年 12 月，年龄 15 |
| 灵力 | 120 ／ 心境(mood) 36 / 100 ／ 心魔(demon) 0 ／ 气血 130/130 |
| 纹银 / 灵石 | 50 / 0 |
| 所在 | 墨大夫药庐（yaolu） |
| 主线指针 storyStage | 6 → **下一节点 `secret_cultivate`（暗藏的锋芒，需练气四层触发）** |
| 已遇 NPC | 张铁、墨大夫、厉飞雨 |
| 关键 flags | at_village, joined_sect, met_modafu, met_friends |
| 背包 | 灵草 ×2 |
| 小绿瓶 | 已解锁，2 块空地 |

**已通过的主线节点**（storyStage 0→5）：
`0 village（青牛镇）` → `1 journey（赴考）` → `2 exam（选拔）` → `3 intro（拜师墨大夫）` → `4 friends（同门之谊·厉飞雨）` → `5 bottle（神秘小绿瓶·解锁）`

**七玄门篇剩余主线节点**（到 `14 arc_end` 本篇完）：
`6 secret_cultivate（暗藏的锋芒）` → `7 zhangtie（挚友失踪/张铁之死）` → `8 showdown_prep（夺舍之谋）` → `9 showdown（夺舍之夜·战）` → `10 take_identity（李代桃僵）` → `11 gang_conflict（野狼帮）` → `12 jinguang_arrives（金光上人）` → `13 jinguang_fight（暗算金光·战）` → `14 arc_end（升仙令·离门，置 arc1_complete，解锁黄枫谷）`
...
- **当前这一步**：我有 2 灵草 + 2 空地。种 2 灵草、打理 3 次收 2 枚灵乳；服 1 枚（200→满 240）即可「尝试突破」练气四层 → 触发 `secret_cultivate`。余 1 枚留下一层用。
