...
- **考据决策（默认 A）**：§3.8① "入门下品法器（烈阳剑）" —— 黄枫谷有据可查的第一件战斗法器=万宝楼二层（练气11，已实装）；练气7~10 无更早"下品飞剑"，"烈阳"是材料（烈阳花）非剑。按红线不臆造、不新增。**「半原创下品飞剑」入 Backlog 待用户拍板**。
...
- **黄枫谷早期"下品飞剑"（半原创）** — 阶段4 §3.8① 暂缓项。考据红线：黄枫谷有据可查的第一件战斗法器=万宝楼二层（练气11）；练气7~10 韩立无更早飞剑，"烈阳"是材料非剑。若用户要"外门标配下品法器"补练气7~10 的法宝轴空窗，需用户先认可名字/设定/驱使门槛，再实装（带 `grantSpells` 的 treasure 招式，minLayer 7~8）。**用户拍板前不动。**
