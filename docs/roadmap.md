...
- **现状审计（2026-06-16·account-2）**：本阶段「未做」标记**准确·非陈旧**。**C 符箓自产=未做**（仅可买符 huoshe_fu/hanbing_fu/jinguang_zhuan + 可买符纸 fu_zhi + 制符笔 zhifu_bi，物品描述明确延后；无方案配方系统 / 大件图鉴无「符」类目 / 无"方案+符纸+灵力即成"自产入口）；**D=部分**（基座「小绿瓶」早于本阶段、七玄门篇已解锁并入大件图鉴 "bottle"，但「随身唤出」仍洞府门禁〔仅 2 洞府地点有 bottle 行动〕、「催熟随境界迁移」未做〔harvestCrop 定额无缩放〕、「多灵草谱」勉强 3 谱；"灵圃"一词全 js/ 无命中=纯文档用词）。用户"好像做了"=小绿瓶基座可见 ≠ 阶段5 交付物。详见 modao-design.md §七审计小节。
