...
> ## ✅ 已修复（v170 · commit d97bbe9 · 2026-06-30 复核确认）
> **三处问题（A 血侍克隆 / B 单场混战缺三战线 / C 皇宫只有地板）已全部解决并上线。**
> - **A**：`bt_xueshi_a/b/c.png` 三差异化立绘已生成入库 + art.js BATTLERS 注册（144-145）；
>   ui.js enemy 渲染分支已改为**优先读 `art` 字段**（4990-4991）；`startSantuanFight` 三血侍
>   异名异型异数值（甲魁梧斧奴铁壁 / 乙枯瘦刺奴高闪 / 丙精悍链奴远程·各带 `art:"xueshi_a/b/c"`）。
> - **B**：`startSantuanFight` 用**声明式 `fronts:[]` 三战区表**（左·刘靖×甲@4 / 中·宋蒙×乙@15 /
>   右·钟卫娘×丙@26，W:30，韩立居中@13），引擎自动落位 + 锁线（本区杀意锁本区同袍）+ 默认开
>   跨场驰援；战斗 UI 现出**「◀ 左·刘靖 / 右·钟卫娘 ▶」战区切换条**（iPhone 14 Pro Max 视口实测）。
...
- [x] 三个血侍**外观各异**（非克隆）。— bt_xueshi_a/b/c + art 字段渲染
- [x] 三组对位在**机制上是三条战线 / 三个战场**，且有**可见的「互相支援」**（跨线驰援 / 韩立瞬移支援）。— fronts:[] 三战区 + 战区切换条
- [x] 皇宫底图有**明确建筑感**（不只是地板），下半幅仍可站位。— huanggong.png 重画
- [x] **14 Pro Max 竖屏视口**下渲染正常，**无字块 / 无默认土地**。— 实测确认
- [x] `?demo=palace` 与正经流程（santuan / tuoshi / xuwang）表现一致。
- [x] `node test/{run,journey,combat}.test.js` 全过；`node scripts/bump.js` 升版（v170）。
